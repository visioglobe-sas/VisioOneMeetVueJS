...
- **`allLocales` can contain locales that aren't meaningfully distinct.** On the shared demo map behind this repo, `venue.translator.allLocales` is `['default', 'en', 'fr']`, and `'default'` turned out to be a byte-identical duplicate of `'fr'` (both French) — confirmed by comparing the published map payload. Don't assume every entry in `allLocales` represents a genuinely different language for a given map; an integrator building a language picker off this list should de-duplicate content, not just locale codes, or at least verify per-map before presenting every code as its own option.
